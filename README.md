...
|-------|---------|-------------|
| Ralph | `/noyeah-ralph` | Persistence loop -- 끝날 때까지 반복 (최대 10회) |
| Autopilot | `/noyeah-autopilot` | Full lifecycle: 인터뷰 -> 계획 -> 구현 -> 검증 |
| Ultrawork | `/noyeah-ultrawork` | Parallel dispatch -- 독립 작업 동시 실행 |
| Ralplan | `/noyeah-ralplan` | Consensus planning: Planner -> Architect -> Critic |
| Ecomode | `/noyeah-ecomode` | Cost modifier -- 에이전트 티어를 한 단계 낮춤 |
| UltraQA | `/noyeah-ultraqa` | QA cycling loop (최대 5라운드) |
| Team | `/noyeah-team` | Multi-agent team coordination |
| Deep Interview | `/noyeah-deep-interview` | Socratic requirements gathering |
| Visual Verdict | `/noyeah-visual-verdict` | Screenshot QA -- 디자인 매칭 검증 |
| Retro | `/noyeah-retro` | Post-completion retrospective |
| Init | `/noyeah-init` | Target project 초기화 |
| Cancel | `/noyeah-cancel` | 실행 중인 모드 종료 |
| Status | `/noyeah-status` | 현재 상태 대시보드 |
| Resume | `/noyeah-resume` | 중단된 작업 재개 |
...
`/noyeah-autopilot`이 전체 파이프라인을 오케스트레이션한다:
...
각 스킬은 독립 실행도 가능하다. `/noyeah-ralph`만 돌려도 되고, `/noyeah-ralplan` + `/noyeah-ralph` 조합도 된다.
