...
1. Run `/cancel` to clean up the failed state
2. Run `/status` to confirm all modes are inactive
3. Analyze what was accomplished: check the files Ralph changed (use `git diff`)
4. Split the task into smaller pieces using `/ralplan`
5. Execute each smaller piece with its own `/ralph` run

**Prevention**: For tasks touching more than 10 files, use `/ralplan` first to break the
...
**Step-by-step resolution**:
1. Run `/cancel` to stop the QA loop
...
4. If environment issue: fix the environment manually, then re-run `/ultraqa`
5. If design flaw: step back and run `/ralplan` to rethink the approach
...
**Step-by-step resolution**:
1. Run `/cancel` to stop the loop
2. Read the architect's rejection reasons carefully (in the Ralph output)
3. Run `/deep-interview "clarify requirements for {task}"` to discover missing requirements
4. Run `/ralplan "{task}"` to create a new plan that addresses the architect's concerns
5. Execute the new plan with `/ralph`

**Prevention**: Always run `/ralplan` for tasks involving 3+ files. The consensus planning
...
5. Fix the issue manually, then resume with `/resume`
...
**Symptom**: `/status` shows unexpected values, skills refuse to start, or error messages
...
**Step-by-step resolution**:
1. Run `/cancel --force` — this resets ALL state files to their terminal states
2. Run `/status` to confirm everything shows as inactive
3. Start your workflow fresh

**Prevention**: Let workflows complete naturally. If you need to stop, use `/cancel`
...
2. Run `/status` to see what state was saved
3. Run `/resume` — this detects the interrupted workflow and continues from the last phase
4. If `/resume` cannot recover, run `/cancel --force` and start the task again
...
- **State files are in `.harness/state/`** — you can read them directly to understand what
  happened: `cat .harness/state/ralph-state.json`
- **Plans are preserved** — even after failure, your plans remain at `.harness/plans/`
  for reference. They are never deleted by `/cancel`.
